...
const AI_ASSISTANT_VERSIONS = [
  { version: 'v0', phaseId: 1, title: 'CLI Chatbot', desc: 'Keyword-matching rule-based bot', tech: 'Python, if-else, CLI', icon: 'chat', level: 'Intermediate' },
  { version: 'v1', phaseId: 3, title: 'API Chatbot', desc: 'FastAPI endpoint wrapping rule-based chatbot', tech: 'FastAPI, Python, REST', icon: 'zap', level: 'Intermediate' },
  { version: 'v2', phaseId: 4, title: 'React UI', desc: 'React chat interface streaming from FastAPI', tech: 'React, WebSockets, FastAPI', icon: 'palette', level: 'Intermediate' },
  { version: 'v3', phaseId: 5, title: 'Data Q&A', desc: 'CSV Q&A assistant answering statistical queries', tech: 'Pandas, NumPy, FastAPI', icon: 'barChart', level: 'Intermediate' },
  { version: 'v4', phaseId: 6, title: 'ML Intent Classifier', desc: 'ML-powered intent classifier triggering skills', tech: 'scikit-learn, SHAP, FastAPI', icon: 'cog', level: 'Advanced' },
  { version: 'v5', phaseId: 7, title: 'LSTM Text Gen', desc: 'LSTM text generator trained on chat logs', tech: 'PyTorch, LSTM, CUDA', icon: 'brain', level: 'Intermediate' },
  { version: 'v6', phaseId: 8, title: 'BERT Intent Router', desc: 'BERT-based intent classifier routing queries', tech: 'HuggingFace, BERT, spaCy', icon: 'chat', level: 'Intermediate' },
  { version: 'v7', phaseId: 9, title: 'Mini GPT', desc: 'NanoGPT-style model trained on your data', tech: 'PyTorch, Transformers', icon: 'gem', level: 'Internship' },
  { version: 'v8', phaseId: 10, title: 'Fine-tuned GPT', desc: 'Fine-tuned nanoGPT on personal chat logs', tech: 'PyTorch, BPE, nanoGPT', icon: 'cog', level: 'Advanced' },
  { version: 'v9', phaseId: 11, title: 'Local 7B Model', desc: 'Fine-tuned 7B model deployed via Ollama', tech: 'LoRA, QLoRA, Ollama', icon: 'target', level: 'Advanced' },
  { version: 'v10', phaseId: 12, title: 'RAG Agent', desc: 'Full RAG agent indexing personal docs & notes', tech: 'LangChain, FAISS, Chroma', icon: 'dna', level: 'Advanced' },
  { version: 'v11', phaseId: 13, title: 'Secure API', desc: 'RAG agent behind streaming API + PostgreSQL', tech: 'FastAPI, JWT, Redis, SSE', icon: 'zap', level: 'Advanced' },
  { version: 'v12', phaseId: 14, title: 'Production AI', desc: 'Fully deployed with Docker, K8s, CI/CD', tech: 'Docker, K8s, MLflow, React', icon: 'rocket', level: 'Advanced' },
];
...
function renderAITimeline(containerId) {
  ...
  container.innerHTML = `
    <div class="career-path">
      <h3 style="font-family:var(--font-heading);font-size:16px;margin-bottom:4px;">${icon('cog')} Personal AI Assistant Evolution</h3>
      ...
      <div style="position:relative;padding-left:24px;">
        ...
        ${AI_ASSISTANT_VERSIONS.map((v, i) => {
          ...
          const statusIcon = isDone ? icon('checkCircle') : isActive ? icon('sync') : '';
          return `
            <div style="position:relative;margin-bottom:16px;${i < AI_ASSISTANT_VERSIONS.length - 1 ? '' : ''}">
              ...
              <div style="background:var(--card);border:1px solid ${isDone ? '#22D3A533' : isActive ? '#F59E0B33' : 'var(--border)'};border-radius:10px;padding:10px 12px;${isDone ? 'border-left:3px solid #22D3A5;' : isActive ? 'border-left:3px solid #F59E0B;' : ''}">
                <div style="display:flex;align-items:center;gap:8px;margin-bottom:4px;">
                  <span style="font-size:16px;">${icon(v.icon)}</span>
