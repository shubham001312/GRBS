// ===== TYPING EFFECT =====
const typingPhrases=["AI Engineer & Full Stack Developer","Building Intelligent Software","Python • ML • LLMs • React • Firebase","Turning Ideas Into Reality","Creator of GRBS & TalkBuzz"];
...
// ===== DARK/LIGHT MODE =====
function toggleTheme(){const body=document.body;const isDark=body.classList.toggle('dark');localStorage.setItem('theme',isDark?'dark':'light');updateThemeIcon(isDark)}
function updateThemeIcon(isDark){const sun=document.querySelector('.sun-icon');const moon=document.querySelector('.moon-icon');if(sun&&moon){sun.style.display=isDark?'none':'block';moon.style.display=isDark?'block':'none'}}
(function(){const saved=localStorage.getItem('theme');const prefersDark=window.matchMedia('(prefers-color-scheme:dark)').matches;if(saved==='dark'||(!saved&&prefersDark)){document.body.classList.add('dark');updateThemeIcon(true)}})();

// ===== SMOOTH SCROLL WITH STAGGER =====
...
async function setupAdminHash(){
  ...
  if(!localStorage.getItem(ADMIN_HASH_KEY)){const h=await sha256('admin@1440');localStorage.setItem(ADMIN_HASH_KEY,h)}
}
setupAdminHash();
...
const sectionDefaults={
  hero:()=>JSON.stringify({name:'Shubham Mallick',headline:'AI Engineer & Full Stack Developer',description:'AI Engineer and developer from Kolkata, building intelligent software at the intersection of AI/ML and modern web development. Creator of GRBS roadmap (15 phases, 500+ resources) and TalkBuzz real-time chat platform. Passionate about LLMs, Computer Vision, and creating tools that make a difference.',tags:['Python','C/C++','Machine Learning','LLMs','Computer Vision','JavaScript','React','Firebase','PWA'],photoUrl:'https://avatars.githubusercontent.com/u/226120019?v=4'},null,2),
